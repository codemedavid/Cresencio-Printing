...
export const useJobOrders = () => {
  ...
  // Keep orders in sync across tabs/windows and after admin updates
  useEffect(() => {
    const handleStorage = (event: StorageEvent) => {
      if (event.key === STORAGE_KEY) {
        try {
          ...
          console.error('Error syncing orders from localStorage:', err);
        }
      }
    };
  ...
  const createOrder = useCallback((orderForm: JobOrderForm, vipMember: VipMember): JobOrder => {
  ...
  // Update order status
  const updateOrderStatus = useCallback((orderId: number, status: JobOrder['status']) => {
    setOrders(prev => 
      prev.map(order => 
        order.id === orderId 
  ...
  // Get order by ID
  const getOrderById = useCallback((orderId: number): JobOrder | undefined => {
